...
| synchdb.dbz_log_level | enum | "warn" | the log level setting for Debezium Runner. Possible values are "debug", "info", "warn", "error", "all", "fatal", "off", "trace". This GUC only determines the initial log level when a connector **starts**; to change the log level of a running connector at runtime without restarting it, use the `synchdb_set_dbz_loglevel(name, level)` function instead, e.g. `SELECT synchdb_set_dbz_loglevel('mysqlconn', 'debug');` |
